...
def initial_sampling(N_ones_wanted_ini):
    ...
    while N_hits_ini < N_ones_wanted_ini: 
        for i in range(2):
            ...
            samples_ini_M1 =  multivariate_normal.rvs(mean = mean[i], cov = [800],size = 1) #sampling_from_IMF(1) #  #np.random.uniform(7, 100,1) #  sampling_from_IMF(1) #change to IS: 
            samples_ini_a =  multivariate_normal.rvs(mean = mean_a[i], cov = [80000],size = 1)
            samples_ini_q = multivariate_normal.rvs(mean = mean_q[i], cov = [0.05],size = 1)
            
            M1_ini_all.append(samples_ini_M1)
            a_ini_all.append(samples_ini_a)
            q_ini_all.append(samples_ini_q)
            
            if  (samples_ini_M1 >100) or (samples_ini_M1 <7) or(samples_ini_a >1000) or (samples_ini_a <0.1): #
                count_outside_parameter_space_ini +=1
                N_ini_all = N_ini_all + 1
                print 'outside parameterspace', count_outside_parameter_space_ini, samples_ini_a
            elif A ==0:
                N_ini_all = N_ini_all + 1
               ...
               # print M1_ini_1
                ...
                gaus_mean_for_ref1.append([samples_ini_M1,samples_ini_a,samples_ini_q[0]])

                #print prior_M1(samples_ini_M1[-1])
                #print ' Hallo Jeroen ik ben hier' #K2[i]*
               # print 'weights'
                #print  prior_M1(samples_ini_M1)
                #print ' prior a', prior_a(samples_ini_a), samples_ini_a #prior_M1(samples_ini_M1)* / (multivariate_normal.pdf(samples_ini_M1,mean = mean[i],cov = [800])
                #print ((multivariate_normal.pdf(samples_ini_M1,mean = mean[i],cov = [800])*multivariate_normal.pdf(samples_ini_a,mean = mean_a[i],cov = [200]) )) 
                w_temp = float((prior*prior2*prior3))/(multivariate_normal.pdf(samples_ini_a,mean = mean_a[i],cov = [80000])*multivariate_normal.pdf(samples_ini_M1,mean = mean[i],cov = [800])*multivariate_normal.pdf(samples_ini_q,mean = mean_q[i],cov = [0.05]) )  #(1./(b_M1-a_M1))
